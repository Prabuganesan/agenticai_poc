...
export class App {
    ...
    async config() {
        ...
        if (process.env.UI_BUILD_PATH) {
            ...
            const packagePath = getNodeModulesPackagePath('autonomous-ui')
            if (!packagePath) {
                const checkedPaths = [
                    path.join(__dirname, '..', 'node_modules', 'autonomous-ui'),
                    path.join(__dirname, '..', '..', 'node_modules', 'autonomous-ui'),
                    path.join(__dirname, '..', '..', '..', 'node_modules', 'autonomous-ui'),
                    path.join(__dirname, '..', '..', '..', 'packages', 'ui'),
                    path.join(process.cwd(), 'packages', 'ui'),
                    path.join(process.cwd(), 'node_modules', 'autonomous-ui')
                ]
                logError(`❌ [server]: autonomous-ui package not found. Checked paths:`).catch(() => {})
                ...
                throw new Error('autonomous-ui package not found')
            }
...
export async function start(): Promise<void> {
    ...
    // Initialize Oracle client if using Oracle database
    // This is the most reliable way to configure the client path, especially on macOS
    if (
        process.env.MAIN_DB_TYPE?.toUpperCase() === 'ORACLE' ||
        (process.env.USE_LOCAL_DB === 'true' && process.env.LOCAL_DB_TYPE?.toUpperCase() === 'ORACLE')
    ) {
        try {
            // Path to the unzipped instant client directory
            const oracleClientPath = path.join(os.homedir(), 'lib', 'instantclient_19_16')
            if (fs.existsSync(oracleClientPath)) {
                ;(oracledb as any).initOracleClient({ libDir: oracleClientPath })
                logInfo(`🔶 [Oracle]: Oracle client initialized from path: ${oracleClientPath}`).catch(() => {})
            } else {
                logWarn(
                    `🔶 [Oracle]: Oracle client path not found at ${oracleClientPath}. The application may fail to connect to Oracle.`
                ).catch(() => {})
            }
        } catch (error) {
            // If client is already initialized, that's fine
            if (error instanceof Error && error.message && error.message.includes('already been initialized')) {
                logInfo('🔶 [Oracle]: Oracle client already initialized').catch(() => {})
            } else {
                logError('❌ [Oracle]: Failed to initialize Oracle client:', error).catch(() => {})
                // Don't throw - allow server to start even if Oracle init fails (might be using PostgreSQL)
            }
        }
    }
