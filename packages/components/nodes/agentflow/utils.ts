 ...
 */
export const updateFlowState = (
    state: ICommonObject,
    updateState: IFlowState[],
    nodeOutput: ICommonObject = {}
): ICommonObject => {
    let newFlowState: Record<string, any> = {}
    for (const stateItem of updateState) {
        let value = stateItem.value

        // Check for {{ output.key }} pattern and replace with actual output value
        if (typeof value === 'string' && value.includes('{{ output.')) {
            const outputRegex = /{{\s*output\.([a-zA-Z0-9_]+)\s*}}/g
            const match = outputRegex.exec(value)

            if (match && match[1]) {
                const key = match[1]
                if (Object.prototype.hasOwnProperty.call(nodeOutput, key)) {
                    // If the value is exactly the key substitution, replace it with the object (preserve type)
                    if (value.trim() === match[0]) {
                        value = nodeOutput[key]
                    } else {
                        // Otherwise, do string substitution
                        value = value.replace(outputRegex, (_match, key) => {
                            return nodeOutput[key] !== undefined ? String(nodeOutput[key]) : ''
                        })
                    }
                }
            }
        ...
        newFlowState[stateItem.key] = value
    }
...
/**
 * Initialize an LLM with callbacks automatically attached
 * This ensures LLM metrics are tracked for agentflows
 *
 * @param modelName - Name of the model to initialize (e.g., 'chatGoogleGenerativeAI')
 * @param modelConfig - Model configuration object containing credentials and settings
 * @param nodeData - Node data containing inputs and other node information
 * @param options - Common options object containing componentNodes, callbacks, etc.
 * @returns Promise<BaseChatModel> - Initialized LLM instance with callbacks attached
 *
 * @example
 * ```typescript
 * const llmInstance = await initializeLLMWithCallbacks(
 ...
 * ```
 */
export async function initializeLLMWithCallbacks(
    modelName: string,
    modelConfig: ICommonObject,
    nodeData: any,
    options: ICommonObject
): Promise<any> {
    // Get the LLM node class from component nodes
    const nodeInstanceFilePath = options.componentNodes[modelName].filePath as string
    ...
    const llmInstance = await newLLMNodeInstance.init(newNodeData, '', options)

    // Attach callbacks from options (passed from buildAgentflow.ts)
    attachCallbacksToLLM(llmInstance, options)

    return llmInstance
}

/**
 * Attach callbacks to an existing LLM instance
 * Use this after bindTools() or other operations that create new instances
 *
 * @param llmInstance - The LLM instance to attach callbacks to
 * @param options - Common options object containing callbacks array
 *
 * @example
 * ```typescript
 * // After bindTools creates a new instance
 * llmInstance = llmInstance.bindTools(tools)
 * attachCallbacksToLLM(llmInstance, options)
 * ```
 */
export function attachCallbacksToLLM(llmInstance: any, options: ICommonObject): void {
    if (!options.callbacks || !Array.isArray(options.callbacks)) {
        return
    }

    // Check if the LLM instance has a callbacks property
    if (llmInstance.callbacks && Array.isArray(llmInstance.callbacks)) {
        // Append to existing callbacks array
        llmInstance.callbacks.push(...options.callbacks)
    } else {
        // Create new callbacks array
        llmInstance.callbacks = [...options.callbacks]
    }
}
