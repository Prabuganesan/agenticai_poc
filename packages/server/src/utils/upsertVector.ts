...
import { logDebug, logError } from '../utils/logger/system-helper'
import {
    buildFlow,
    constructGraphs,
...
import { IncomingInput, INodeDirectedGraph, IReactFlowObject, IExecuteFlowParams, MODE } from '../Interface'
...
export const executeUpsert = async ({
    ...
    // Find the upsert result from the executed nodes
    const stopNode = upsertedResult.find((node: any) => node.data.id === stopNodeId)
    const upsertResult = stopNode?.data?.instance

    // Save to DB
    if (upsertResult && typeof upsertResult === 'object' && 'flowData' in upsertResult && 'result' in upsertResult) {
        const { generateGuid } = await import('./guidGenerator')
        const result: any = cloneDeep(upsertResult)
    ...
    return upsertResult?.result ?? { result: 'Successfully Upserted' }
}
