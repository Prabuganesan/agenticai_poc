...
const CredentialInputHandler = ({ inputParam, data, onSelect, disabled = false }) => {
    ...
    const [credentialId, setCredentialId] = useState(data?.credential || (data?.inputs && data.inputs[KODIVIAN_CREDENTIAL_ID]) || '')
    ...
    useEffect(() => {
        setCredentialId(data?.credential || (data?.inputs && data.inputs[KODIVIAN_CREDENTIAL_ID]) || '')
    }, [data])
